...
Enterprise.statusExplain = function (status) {
    var text = '系统繁忙，请稍后再试！';
    switch (status) {
        case 400:
            text = '客户端错误';
 ...
 * @constructor
 */
Enterprise.ApiProxy = function (urlSuffix, data, text, isPost) {
    var url = Enterprise.path + urlSuffix;
    ...
    $.ajax(url, option).then(function (result) {
        ...
        if(Enterprise.isPublish) { // 发布环境
            ...
            var errorText = Enterprise.statusExplain(XMLHttpRequest.status);
            if(errorText == '未知错误') {
...
//  * @constructor
//  */
// Enterprise.ApiProxyList = function (urlSuffix, data, text) {
//     var url = Enterprise.path + urlSuffix;
//
//     var dfd = $.Deferred();
 ...
 * @constructor
 */
Enterprise.ApiProxyJson = function (url, data, text) {
 ...
 * @returns {*}
 */
Enterprise.upLoadFile = function ($elemnt) {
    var url = Enterprise.path + '/file_upload';
    var dfd = $.Deferred();
 ...
 * @returns {*}
 */
Enterprise.getSessionAccount = function (params) {
    return Enterprise.ApiProxy('/login/getSessionAccount', params, '获取当前登录信息');
};
 ...
 * @constructor
 */
Enterprise.Login = function (params) {
    return Enterprise.ApiProxy('/login/Login', params, '用户登录');
};
 ...
 * @returns {*}
 */
Enterprise.exitAccount = function (params) {
    return Enterprise.ApiProxy('/login/exit', params, '退出登录');
};
 ...
 * @returns {*}
 */
Enterprise.limit = function (params) {
    return Enterprise.ApiProxy('/bg/role/limit', params, '获取导航菜单列表');
};
