...
$(function () {
    ...
    function renderList(isClear) {
        // 获取组织列表
        CadreApi.getCadreList(params).then(function (data) {
            console.log('CadreApi.getCadreList data', data);
            var list = data.rows;
            var html = '';
            ...
            for(var i=0; i<list.length; i++) {
                ...
                html += '<a class="table_con table_tr clearfix" href="cadre_detail.html?tid=' + item.tid + '" style="display: block;">';
                html += '    <span class="td">' + index_global + '</span><span class="td">' + item.name + '</span><span class="td">' + item.createName + '</span><span class="td">' + item.mobile + '</span>';
                html += '</a>';
            }
