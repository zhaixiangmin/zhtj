...
$(function () {
    ...
    $('.ul').on('click', 'li', function () {
        ...
        if(oidName.indexOf('退回原籍团支部') != -1) {
            $.alert('您好，团员报到不支持报到至退回原籍团支部，请选择您当前所在的具体团支部，请知悉。');
            return;
        }

        if(oidName.indexOf('非共青团广东省委所辖的团组织') != -1) {
            $.alert('您好，团员报到不支持报到至非团广东省委所辖组织，请重新选择广东省内的具体团支部，谢谢。');
            return;
        }
        ...
        if($.cookie) {
            ...
            zhtj['oid'] = oid;
            zhtj['oidName'] = oidName;
            zhtj['oidIsChosen'] = true;

            $.cookie('zhtj', JSON.stringify(zhtj), {path: '/'}); // 存储到cookie
            window.location.href = 'phone_bind.html'; // 跳转
        }
    });
});