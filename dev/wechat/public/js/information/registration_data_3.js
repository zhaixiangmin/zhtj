 ...
 */
$(function () {
    var optionList_global = ['nation', 'nationInfo', 'politicalOutlook', 'degreeOfEducation', 'highestEducation', 'residence', 'mobile', 'leagueForUnit']; // 参数列表(全局变量)
    ...
    if($.cookie && $.cookie('zhtj')) {
        ...
        for(var i=0; i<optionList_global.length; i++) {
            ...
            if(option == 'nation') { // 民族
                $('#' + option).find('.arrow').text(nationName[zhtj[option]]);
                $('#' + option).find('.arrow').data('id', zhtj[option]);
                if(zhtj[option] == 57) { // 其他(民族)
            ...
            if(option == 'nationInfo') { // 民族名称
                $('#' + option).find('input').val(zhtj[option]);
            }
            if(option == 'politicalOutlook') { // 政治面貌
                $('#' + option).find('.arrow').text(politicalOutlookName[zhtj[option]]);
                $('#' + option).find('.arrow').data('id', zhtj[option]);
                continue;
            }
            if(option == 'degreeOfEducation') { // 全日制学历
                $('#' + option).find('.arrow').text(degreeOfEducationName[zhtj[option]]);
                $('#' + option).find('.arrow').data('id', zhtj[option]);
                continue;
            }
            ...
            if(option == 'residence') { // 户籍所在地
                $('#' + option).find('.arrow').text(zhtj[option]); // 城市名称
                $('#' + option).find('.arrow').data('provinceDid', zhtj['provinceDid']); // 省份ID
                $('#' + option).find('.arrow').data('cityDid', zhtj['cityDid']); // 市级ID
                $('#' + option).find('.arrow').data('countyDid', zhtj['countyDid']); // 县级市级ID
                continue;
            }
            if(option == 'leagueForUnit') {
                $('#' + option).find('input').val(zhtj[option]); // 入团时所在单位
                continue;
            }
            // $('#' + option).find('input').val(zhtj[option]); // 手机号码
        }
    }
    ...
    InformationApi.MyProfile({}).then(function (data) {
        ...
        if (myData) {
            var mobile = myData.mobile; // 手机号码
            $('#mobile .right_txt').text(mobile); // 设置 手机号码
        }
    });

    var currentSelectName_global = undefined; // 当前选择框名称(全局变量，nation：民族，politicalOutlook：政治面貌，degreeOfEducation：全日制学历，highestEducation：最高学历)
    ...
    // 点击'可选择项'(可弹出框) -- 民族/政治面貌/全日制学历/最高学历
    $('#nation, #politicalOutlook, #degreeOfEducation, #highestEducation').click(function () {
        var list = undefined;
        var text = $(this).attr('id');
        // if(!text || text == 'mobile' || text == 'residence') { // 空
        if(!text) { // 空
            return;
        }
        currentSelectName_global = text; // 当前选择框名称(全局变量，nation：民族，politicalOutlook：政治面貌，degreeOfEducation：全日制学历，highestEducation：最高学历)
        if(text == 'nation') { // 民族
            list = list_nation;
        }else if(text == 'politicalOutlook'){ // 政治面貌
    ...
    $('.select_popup .select_list').on('click', '.select_item', function(event) {
        ...
        }else if(currentSelectName_global == 'nation') { // 民族
    ...
    $('.select_popup_pair .select_list_pair').on('click', '.select_item_pair', function(event) {
        ...
        if(isLeft) { // 当前是左边(省份)
            ...
            // $('#residence').find('.arrow').data('provinceName', $(this).text()); // 省份名称
            if(!did) {
                $.alert('当前省份参数为空');
                return;
            }
            ...
            // $('#residence').find('.arrow').data('provinceDid', $('.select_popup_pair .select_list_pair.select_list_pair_left .select_item_pair.active').data('did')); // 省份ID
            // $('#residence').find('.arrow').data('cityDid', cityDid); // 市级ID
            ...
            InformationApi.getDistrictByPid({did: cityDid}).then(function (data) {
                var list = data.rows;
                // initialSelectPair(list_right, $('.select_popup_pair .select_list_pair.select_list_pair_right')); //渲染页面(选择弹出框 - 两边) - 右边
                currentSelectName_global = 'residence'; // 当前选择框名称(全局变量，nation：民族，politicalOutlook：政治面貌，degreeOfEducation：全日制学历，highestEducation：最高学历)
    ...
    $('.bot_big_btn').click(function () {
        var params_info = {
            mobile: $('#mobile .right_txt').text().trim(), // 手机号码
            nation: $('#nation').find('.arrow').data('id'), // 民族
            nationInfo: $('#nationInfo input').val(), // 民族名称
            ...
            residence: $('#residence').find('.arrow').text(), // 户籍所在地名称
        ...
        if(!params_info.provinceDid || !params_info.cityDid) {
            $.alert('请选择户籍所在地');
            return;
        }
        ...
        if($.cookie && $.cookie('zhtj')) {
            ...
            window.location.href = 'registration_data_4.html'; // 跳转
        }
    });
